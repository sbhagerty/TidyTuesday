...
##Join all 3 together
#get player dob in a standardized format
...
## Ideal df has Name, DOB, Date of Tournament, Win Count, for all grand slam winners 
ggplot(df, aes(x=playerAge, y=rolling_win_count, group=player))+geom_line()+facet_grid(.~gender)
...
X<-ggplot(df, aes(x=playerAge, y=rolling_win_count, color=name))+geom_line()+ 
  geom_point(size = 2)+ xlim(15,40)+ylab('Grand Slam Wins')+
  geom_label_repel(label = df$name, hjust=1, xlim=c(35,NA),direction='y',segment.color='gray50')+
  facet_grid(gender~.)+xlab('Age')+
